...
The instructions each have one of the following forms:
   ...
   ...
   <<<
          Run Main with COMMAND ARGUMENTS as its parameters.  Compare
          ...
          or JAR file containing the Main program is assumed to be
...
GITLET_CLASS = "edu.northeastern.gitlet.Gitlet"
...
def doExecute(cmnd, dir, timeout, line_num):
    ...
    try:
        ...
        return ("java Main exited with code {}".format(excp.args[0]),
                excp.output)
...
if __name__ == "__main__":
    ...
    try:
        ...
        if prog_dir is None:
            ...
            while k > 0 and access(prog_dir, W_OK):
                k -= 1
                if exists(join(prog_dir, 'target/classes/edu/northeastern/gitlet', 'Gitlet.class')):
                    break
                prog_dir = dirname(prog_dir)
            else:
                print("Could not find Gitlet.", file=sys.stderr)
                sys.exit(1)
    except GetoptError:
        Usage()
    if not files:
    ...
    class_path = prog_dir + "/target/classes"
    ON_WINDOWS = Match(r'.*\\', join('a', 'b'))
    if ON_WINDOWS:
        if 'CLASSPATH' in environ:
            environ['CLASSPATH'] = "{};{}".format(class_path, environ['CLASSPATH'])
        else:
            environ['CLASSPATH'] = "{}".format(class_path)
    else:
        if 'CLASSPATH' in environ:
            environ['CLASSPATH'] = "{}:{}".format(class_path, environ['CLASSPATH'])
        else:
